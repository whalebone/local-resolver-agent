...
class Tester():
    ...
    def search_redis(self, uid: str):
        if self.redis.exists(uid):
            data = json.loads(self.redis.get(uid).decode("utf-8"))
    ...
    def upgrade_resolver(self, name: str):
        ...
        try:
            ...
            for _ in range(self.max_retry):
                ...
                self.status["upgrade_resolver_{}".format(name)] = {"fail": "timeout"}
        ...
        try:
            ...
            try:
                ...
                for _ in range(self.max_retry):
                    ...
                    self.status["upgrade_resolver_not_present"] = {"fail": "timeout"}
        ...
        try:
            ...
            try:
                ...
                for _ in range(self.max_retry):
                    ...
                    self.status["upgrade_resolver_with_old"] = {"fail": "timeout"}
    ...
    def upgrade_all(self, services: list, scenario: str):
        ...
        try:
            ...
            for _ in range(self.max_retry + 1):
                ...
                status = self.search_redis(uid)
                if status:
                    self.logger.warning("Case {}, {}".format(scenario, status))
                    if not os.path.exists("/etc/whalebone/agent/upgrade.json"):
                        if all(service_status["status"] == "success" for service_status in status["action_status"].values()):
                            self.logger.info("Upgrade Received")
                            self.status["upgrade_all_{}".format(scenario)] = "ok"
                            self.check_named_volume()
                            break
            else:
    ...
    def check_named_volume(self):
        if "dnstap" not in [volume.name for volume in self.docker_client.volumes.list()]:
            ...
            for container in self.docker_client.containers.list():
                if container.name in ("resolver", "dnstag"):
                    if not any("Name" in volume and volume["Name"] == "dnstap" for volume in container.attrs["Mounts"]):
                        self.logger.warning("Container not bound {} to volume.".format(container.name))
    ...
    def get_sysinfo(self):
        try:
            ...
            containers = {"lr-agent", "resolver", " logrotate", "passivedns", "logstream", "kresman"}
            try:
                for key, value in rec.json()["action_status"].items():
                    if key == "containers":
                        self.logger.info("Containers: {}".format(value))
                        if set(containers).issubset(set(value.keys())):
                            self.logger.info("All services are up")
                    if key in (
                    "cpu", "memory", "hdd", "network_info", "disk_iops", "kresman", "images", "kresman_internal"):
    ...
    def test_inactive_action(self):
        try:
            ...
            rec = requests.post(
                "http://{}:8080/wsproxy/rest/message/{}/remove".format(self.proxy_address, self.agent_id),
                json={"containers": ["passivedns"], "uid": uid})
        except Exception as e:
            self.logger.info(e)
        else:
            for _ in range(self.max_retry):
                ...
                if status:
                    try:
                        if status["action_status"]["message"] == "Action 'remove' is not supported.":
                            ...
                            self.status["inactive_action"] = {"fail": status}
                    except KeyError:
                        self.logger.info("Inactive action failed to be detected")
                        self.status["inactive_action"] = {"fail": status}
                    break
                ...
                self.status["inactive_action"] = {"fail": "timeout"}
    ...
    def update_cache(self):
        try:
            ...
            for _ in range(self.max_retry):
                status = self.search_redis(uid)
                if status:
                    if status["action_status"]["status"] == "success":
                ...
                self.status["update_cache"] = {"fail": "timeout"}
    ...
    def clear_cache(self):
        ...
        try:
            rec = requests.post(
                ...
                json={"clear": "all", "uid": uid})
        except Exception as e:
            self.logger.info(e)
        else:
            rec = rec.json()
            try:
                if rec["action_status"]["status"] == "success":
    ...
    def trace_domain(self):
        try:
            ...
            rec = requests.post(
                ...
                json={"domain": "whalebone.io", "query_type": "A", "uid": uid})
        except Exception as e:
            self.logger.info(e)
        else:
            rec = rec.json()
            try:
                ...
                self.logger.warning("Trace failed due to {}".format(rec))
    ...
    def upgrade_resolver_config(self):
        ...
        try:
            ...
            for _ in range(self.max_retry):
                ...
                self.status["upgrade_config"] = {"fail": "timeout"}
    ...
    def upgrade_resolver_redirect(self):
        ...
        try:
            ...
            for _ in range(self.max_retry):
                ...
                self.status["upgrade_redirect"] = {"fail": "timeout"}
    ...
    def upgrade_resolver_image(self):
        ...
        try:
            ...
            for _ in range(self.max_retry):
                ...
                self.status["upgrade_image"] = {"fail": "timeout"}
    ...
    def pack_data(self):
        try:
            ...
            rec = requests.post(
                ...
                json={"url": "http://localhost:8000/slack", "uid": uid})
        except Exception as e:
            self.logger.info(e)
        else:
            for _ in range(self.max_retry):
                status = self.search_redis("datacollect")
                if status and uid not in status:
                    self.logger.info(status)
                ...
                self.status["pack_data"] = {"fail": "timeout"}
    ...
    def clean_directory(self, directory: str):
        for log in os.listdir(directory):
            try:
                os.remove("{}/{}".format(directory, log))
            except Exception:
                pass
